...
filename = "12lateralNew90.csv";
file = open(filename,"w")
writecsv(file, ["M" "BFL1" "BFV1" "GAL1" "GAV1" "BCL1" "BCV1" "BFL2" "BFV2" "GAL2" "GAV2" "BCL2" "BCV2" "BFL3" "BFV3" "GAL3" "GAV3" "BCL3" "BCV3" ]);
for M = 0:N
  i = 0*(3N+3) + M+1
  writecsv(file, [M bfls[i] bfvs[i] gals[i] gavs[i] bcls[i] bcvs[i] bfls[i+N+1] bfvs[i+N+1] gals[i+N+1] gavs[i+N+1] bcls[i+N+1] bcvs[i+N+1] bfls[i+2N+2] bfvs[i+2N+2] gals[i+2N+2] gavs[i+2N+2] bcls[i+2N+2] bcvs[i+2N+2]]);
end
close(file)


filename = "12lateralNew50.csv"
file = open(filename,"w")
writecsv(file, ["M" "BFL1" "BFV1" "GAL1" "GAV1" "BCL1" "BCV1" "BFL2" "BFV2" "GAL2" "GAV2" "BCL2" "BCV2" "BFL3" "BFV3" "GAL3" "GAV3" "BCL3" "BCV3" ]);
for M = 0:N
  i = 1*(3N+3) + M+1;
...
filename = "12lateral50.csv";
file = open(filename,"w")
writecsv(file, ["M" "BCL" "BCV" "BCAL" "BCAV"]);
for i = 0:N
  i = 0*(N+1) + M+1
  writecsv(file, [M bfls[i] bfvs[i] gals[i] gavs[i] bcls[i] bcvs[i] bfls[i+N+1] bfvs[i+N+1] gals[i+N+1] gavs[i+N+1] bcls[i+N+1] bcvs[i+N+1] bfls[i+2N+2] bfvs[i+2N+2] gals[i+2N+2] gavs[i+2N+2] bcls[i+2N+2] bcvs[i+2N+2]]);
end
